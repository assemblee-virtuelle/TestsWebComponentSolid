...
class PlanetHandler{
    ...
    deletePerm(toDelete, uri){
        let aclUri = this.getAclUri(uri);
        return new Promise((resolve, reject) => {
            let parsed = this.parseDeleteToSparql(toDelete, uri);
            this.account.fetch(aclUri, {
                method:'PUT',
                headers: {'Content-Type':'text/turtle'},
                body:parsed
