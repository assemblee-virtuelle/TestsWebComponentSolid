...
class PlanetHandler{
    ...
    /**
     * @description Creates the planet list at the root folder (this.account.uri)
     * and its default ACL
     */
    createPlanetList(){
        return new Promise((resolve, reject) => {
    ...
    checkNameAvailability(name){
        for (const key in this.planetList) {
            if (this.planetList.hasOwnProperty(key)) {
                let test = this.planetList[key].planet.inputs[PLANET('name')];
        ...
        return new Promise((resolve, reject) => {
            ...
            if (uri != this.pathToList){
                if (!this.planetList[uri]){
                    reject("Invalid Uri or planet not found");
                }
                if (this.planetList[uri] && !this.planetList[uri].acl){
                    reject("No acl linked for this resource");
                }
                aclFile = this.pathToList + this.planetList[uri].acl;
            } else {
    ...
    addNewPerm(permissions, uri){
        let parsed = this.parsePermsToSparql(permissions, uri);
        ...
        return new Promise((resolve, reject) => {
            if(!this.webidCheck(permissions.webid, uri)){
                this.account.fetch(aclUri, {
                    ...
                    if(res.status == 200){
                        //Not necessary because list is updated each add new perm and delete perm in main.js
                        // if (uri !== this.pathToList){
                        //     let store = $rdf.graph();
                        //     this.planetList[uri].hasAcl = true;
                        //     let rdfQuery = $rdf.SPARQLToQuery(query, false, store);
                        //     store.addAll(rdfQuery.pat.statements);
                        //     this.planetList[uri].permissions.store = store;
                        // } else {
                        //     this.planetFolderPerms.push(uri);
                        // }
                        resolve();
                    } else {
                        reject(res.status);
    ...
    parsePermsToSparql(permissions, resource){
        let query = `INSERT {`;
        ...
        //TODO: check if webid is taken or not
    ...
    getAclUri(uri){
        ...
        if (uri === this.pathToList){
            ...
            aclUri = this.pathToList + this.planetList[uri].acl;
        }
        return aclUri;
    }
