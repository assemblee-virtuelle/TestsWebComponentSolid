...
class PlanetHandler{
    ...
    fetchPlanetList(){
        return new Promise((resolve, reject) => {
            ...
            .catch(err => {
                if (err == 404){
                    this.createPlanetList().then(res => resolve({list:null, listUri:this.pathToList, uri:this.account.accountUri}))
                    .catch(err => reject(err));
                } else {
                    reject(err);
                }
                ...
                if(!exists){
                    ...
                    if(decoded['triples'] && decoded['triples'] != ""){
                        ...
                        .then(h => {
                            ...
                            let uri = this.serverList + relativeUri;
                            let newPlanet = {};
                            let inputs = {};
                            ...
                            return this.createDefaultPlanetAcl(uri);
                        })
                        .then(res => resolve())
                        .catch(err => reject(err));
                    }
    ...
    createDefaultPlanetAcl(resource){
        let aclUri = resource + this.aclSuffix;
        return new Promise((resolve, reject) => {
            let query = "INSERT {";
            query += `${$rdf.sym(aclUri + "#Owner")} ${RDF('type')} ${ACL('Authorization')};
            ${ACL('agent')} ${$rdf.sym(this.account.webid)};
    ...
    addNewPerm(permissions, uri, edit){
        let parsed = this.parseNewPermsToSparql(permissions, uri, edit);
    ...
    editPerm(permissions, uri){
        return new Promise((resolve, reject) => {
            let aclUri = this.getAclUri(uri);
            let query = this.editPermToSparql(permissions, uri);
            this.account.fetch(aclUri, {
                method:'PUT',
                headers: {'Content-Type':'text/turtle'},
                body:query
    ...
    editPermToSparql(permissions, resource){
        let webid = permissions.webid;
        let aclStore = null;
        let aclUri = this.getAclUri(resource);
        if (resource !== this.pathToList){
            aclStore = this.planetList[resource].permissions.store;
        } else {
        ...
        for(let i = 0; i < subjList.length; i++){
            let modeList = aclStore.statementsMatching(subjList[i].subject, ACL('mode'), undefined)
            for(let j = 0; j < modeList.length; j++){
                aclStore.remove(modeList[j])
            }
            for(let k = 0; k < permissions.modes.length; k++){
                aclStore.add(subjList[i].subject, ACL('mode'), ACL(permissions.modes[k]));
            }
        }
